...
from analysis import news_accuracy as news_analysis
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_clas', type=int, default=2)
    ...
    ((trX1, trX2, trY), (vaX1, vaX2, vaY), (teX1, teX2)) = \
        encode_dataset(*news(data_dir, n_valid=args.n_valid), encoder=text_encoder)
    ...
    trX, trM = transform_news(trX1, trX2)
    vaX, vaM = transform_news(vaX1, vaX2)
    if submit:
        teX, teM = transform_news(teX1, teX2)
    ...
    dh_model = DoubleHeadModel(args, clf_token, ('classification', n_clas), vocab, n_ctx)
    ...
    if submit:
        ...
        if args.analysis:
            news_analysis(data_dir, os.path.join(args.submission_dir, desc+'.tsv'),
                                os.path.join(log_dir, desc+'.jsonl'))
